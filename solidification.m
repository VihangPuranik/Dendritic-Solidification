...
alpha = 0.9;
a = 0.00;
J = 6; % Mode of anisotropy
delta = 0.05;
eps_bar = 0.005;

T_new = zeros(size,size2);
p_new = p;
time = 1; % Total time for simulation
        ...
        for i = 2:size-1
            for j = 2:size2-1
                ...
                if (p(i,j)<=0.5 && p(i,j)>=0)
                    p_new(i,j) = p(i,j)
                    + (part1 + part2 + part3 + eps(i,j)^2 * (p(i+1,j) + p(i-1,j) + p(i,j+1) + p(i,j-1) - 4*p(i,j))/(del_x*del_y)
                    + p(i,j)*(1-p(i,j))*(p(i,j)-0.5+m))*del_t/tou
                    + a*p(i,j)*(1-p(i,j))*(rand(1)-0.5);
                ...
                T_new(i,j) = T(i,j)
                + (T(i+1,j) + T(i-1,j) + T(i,j+1) + T(i,j-1) - 4*T(i,j)) * del_t/del_x^2
                + K*(p_new(i,j)-p(i,j));
            end
        end
